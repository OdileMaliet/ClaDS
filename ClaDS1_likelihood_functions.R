...
Phi_ClaDS1=function(sigma,alpha,mlambda,Mlambda,nlambda,mu,f,tini=0,tf=100,by=0.1,method="Higham08.b"){
...
createLikelihood_ClaDS1 <- function(phylo, root_depth=0, relative = F,nt=1000,nlambda=100,method="FFT",banded=T,
                                         mlambda=1e-5,Mlambda=1e2,interval=5,verbose=F,conv=1e-10,div=10){
  ...
  ll<-function(param,f,former=NULL){
    ...
    Mlambda2=min(log(Mlambda),(max(lambda2)+3*max(sigma,la)))
    # Mlambda2=exp(log(2)*ceiling(log(Mlambda2)/log(2)))
    Mlambda2=exp(ls*ceiling((Mlambda2)/(ls)))
    
    mlambda2=max(log(mlambda),(min(lambda2)+3*min(-1*sigma,la)))
    ...
    if(eval){
      ...
      phi=Phi_ClaDS1(sigma,alpha,mlambda2,Mlambda2,nlambda2,mu,f,tf=tf,by=tf/nt,method=method)
      for(i in 1:(length(lambda2)-1)){
        if(type[i]==1){
          m=Khi_ClaDS1(phi,0,nodeprof[phylo$edge[i,1]],nt=nt,method=method,banded = banded,func = "Khi",interval=interval,conv=conv)
          ...
          m=Khi_ClaDS1(phi,nodeprof[phylo$edge[i,2]],nodeprof[phylo$edge[i,1]],lambda1 = lambda2[offspring[1,i]+1],
                lambda2 = lambda2[offspring[2,i]+1],func = "Ksi",nt=nt,method=method,banded = banded,interval=interval,conv=conv)
          ind=which.min(abs(lambda2[i+1]-phi$lambda))
          PsiKhi[i+1]=InterpolatedKhi(phi,m,lambda2[i+1])
        }} 
      if(root_depth==0){
      ...
      for( i in change){
        if(i ==0){
          if(root_depth==0){
            ...
            m=Khi_ClaDS1(phi,root_depth+nodeprof[nbtips+1],nodeprof[nbtips+1],lambda1 = lambda2[roots[1]+1],
                  lambda2 = lambda2[roots[2]+1],func = "Zeta",nt=nt,method=method,banded = banded,interval=interval,conv=conv)
            ind=which.min(abs(lambda2[1]-phi$lambda))
            PsiKhi[1]=log(m[ind])
          }
        }else{
